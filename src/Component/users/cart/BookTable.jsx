...
export default function BookTable({cartList,cartId,setCartId,config,quantity,setQuantity}){
    const decreaseBookQuantity = (cartItemID,cartQuantity) =>{
        cartQuantity -=1;
        console.log(cartQuantity)
        setQuantity(cartQuantity);
        changeQuantity(cartItemID,cartQuantity);
    }

    const increaseBookQuantity = (cartItemID,cartQuantity) => {
        cartQuantity +=1;
        console.log(cartQuantity)
        setQuantity(cartQuantity);
        changeQuantity(cartItemID,cartQuantity);
    ...
    const changeQuantity = (cartItemID,quantity) =>{
        axios.put("https://ecommerce-web0903.herokuapp.com/api/carts/"+cartItemID+"/change?quantity="+quantity,null,config)
            .then((res)=>{
                console.log(res);
            })
            .catch((error) =>{
                ...
                <Row key={item.cartItemsID}>
                    ...
                    <Col span={24}>
                        ...
                        <Row style={{paddingBlock:"4%"}}>
                            ...
                            <Col span={4}>
                                <Col span={19}>
                                    <CartButton setQuantity={setQuantity} quantity={item.quantity} cartItemID={item.cartItemsID} config={config} />
                                </Col>
                            </Col>
                            {/*---------------------------Header--------------------------------*/}
                            <Col span={4}>
